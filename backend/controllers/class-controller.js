...
const getSclassStudents = async (req, res) => {
    try {
        let parents = await Parent.find({ sclassName: req.params.id })
        if (parents.length > 0) {
...
const deleteSclass = async (req, res) => {
    try {
        ...
        const deletedStudents = await Parent.deleteMany({ sclassName: req.params.id });
...
const deleteSclasses = async (req, res) => {
    try {
        ...
        const deletedStudents = await Parent.deleteMany({ school: req.params.id });
